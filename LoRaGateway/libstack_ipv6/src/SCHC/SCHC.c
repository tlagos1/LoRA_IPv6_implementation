...
char *icmp_decompression(char *schc_buffer, char *buffer, int schc_offset, int buffer_offset, int packet_size, uint8_t *mac)
{
    uint16_t payload_length = 0;
    uint16_t checksum = 0;

    int icmp = (schc_buffer[0] >> 2) & 7;
    int schc_src = (schc_buffer[0] >> 6) & 1;
    int schc_target = (schc_buffer[0] >> 1) & 1;
    int ix;
    // type
    if(icmp == 0)
    {
    ...
    {
        //src addr
        memcpy(&buffer[16], &schc_buffer[schc_offset], 8);
        ...
        for(ix = 0; ix < 6; ix++)
        {
            buffer[buffer_offset + ix] = mac[ix];
        }
        buffer_offset += 6;
    }
    ...
    else if(icmp == 4) // if it is neighbor solicitation
    {
        ...
        {
            memcpy(&buffer[8], &schc_buffer[schc_offset], 16);
            schc_offset += 16;
        }
        ...
        if(schc_target == 0)
        {
            memset(&buffer[buffer_offset], 0, 16);
            buffer[buffer_offset] = 0xfe;
            ...
            for(i = 0; i < 8; i++)
            {
                buffer[buffer_offset + i] = schc_buffer[schc_offset + i];
            }
            buffer_offset += 8;
        }
        else
    ...
    else if(icmp == 5) // if it is neighbor advertisement
    {
        ...
        // flags
        buffer[buffer_offset] = 0;
        buffer[buffer_offset] |= 1 << 7; // 1 is router - 0 is node
        buffer[buffer_offset] |= 1 << 6; // 1 is neighbor solicitation answer
        buffer[buffer_offset] |= 0 << 5; // 1 rewrite cache from source device - 0 else
        buffer[buffer_offset+1] = 0;
        buffer[buffer_offset+2] = 0;
        buffer[buffer_offset+3] = 0;
        buffer_offset += 4;  
...
char *schc_decompression(char *schc_buffer, char *buffer, int packet_size, uint8_t *src, uint8_t *dst, uint8_t *mac)
{
    int schc_offset = 1;
    int buffer_offset = 0; 
    int ix;
    ...
    if(src != NULL)
    {
        for(ix = 0; ix < 16; ix++)
        {
            buffer[buffer_offset + ix] = src[ix];
        }
        buffer_offset += 16;
    }
    else
    {
        if(schc_src == 0) // => source address link local
        {
            ...
            if(schc_header == 0)
            {
                memset(&buffer[buffer_offset], 0, 8);
                buffer_offset += 8;
            }
        }
    ...
    if(dst != NULL)
    {
        for(ix = 0; ix < 16; ix++)
        {
            buffer[buffer_offset + ix] = dst[ix];
        }
        buffer_offset += 16;
    }
    else
    ...
    if(schc_header == 0)
    {
        icmp_decompression(schc_buffer, buffer, schc_offset, buffer_offset, packet_size, mac);
    }
    return buffer;
}
